...
void repository_destroy(Repository* repository) {
    vector_destroy(repository->data, country_destroy);
    free(repository);
}
...
int repository_remove_country(Repository* repository, Country* country_to_remove) {
    ...
    for (int i = 0; i < repository_size; ++i) {
        if (strcmp(get_name(vector_get_item(repository->data, i)), get_name(country_to_remove)) == 0 &&
            ...
            set_name(vector_get_item(repository->data, i), get_name(vector_get_item(repository->data, repository_size - 1)));
            set_continent(vector_get_item(repository->data, i), get_continent(vector_get_item(repository->data, repository_size - 1)));
            set_population(vector_get_item(repository->data, i), get_population(vector_get_item(repository->data, repository_size - 1)));
